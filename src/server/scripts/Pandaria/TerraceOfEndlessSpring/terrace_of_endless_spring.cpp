...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (sConfigMgr->GetIntDefault("FirstOfTheKingdom.TerraceSpring", 1))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Turn off dampening?", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    ...
    struct npc_terrace_springtender_ashaniAI : public ScriptedAI
    {
        ...
        {
            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_LFR)
                ...
                if (corruptionBoltTimer)
                {
                    if (corruptionBoltTimer <= diff)
                    {
                        ...
                        if (instance)
                        {
                            ...
                            if (Unit* target = ObjectAccessor::FindUnit((*targets.begin())))
