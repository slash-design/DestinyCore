...
struct scenario_the_violet_hold : public InstanceScript
{
    ...
    void OnPlayerEnter(Player* player) override
    {
        ...
        SendScenarioState(ScenarioData(1034, STEP_0));
        RespawnCreature(0);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case DELAY_EVENT_2:
                if (InstanceScript* l_InstanceScript = me->GetInstanceScript())
                {
                    l_InstanceScript->DoConversation(1247);
                }
                break;
            case EVENT_PART3:
...
struct npc_forge_of_the_guardian_102786 : public ScriptedAI
{
    ...
    void OnSpellClick(Unit* who, bool& /*result*/) override
    {
        ...
        me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_SELECTABLE);//| UNIT_FLAG_NOT_SELECTABLE
    }
