...
float LFGListMgr::GetPlayerItemLevelForActivity(GroupFinderActivityEntry const* activity, Player* player) const
{
    ...
    return player->GetAverageItemLevelEquipped() + (IsActivityPvP(activity) ? PlayerAvgItemLevelOffsets::PLAYER_AVG_ITEM_LEVEL_UNK4 : PlayerAvgItemLevelOffsets::PLAYER_AVG_ITEM_LEVEL_UNK3);
}
