...
typedef std::unordered_map<uint64, PlayerVignette> PlayerVignettesMap;
...
{
    TRANSFER_ABORT_ZONE_IN_COMBAT                   = 0x00,         // Unable to zone in while an encounter is in progress.
    TRANSFER_ABORT_NOT_FOUND                        = 0x02,         // Transfer Aborted: instance not found
    TRANSFER_ABORT_LOCKED_TO_DIFFERENT_INSTANCE     = 0x03,         // 4.2.2
    TRANSFER_ABORT_SOLO_PLAYER_SWITCH_DIFFICULTY    = 0x05,
    TRANSFER_ABORT_UNIQUE_MESSAGE                   = 0x06,         // Until you've escaped TLK's grasp, you cannot leave this place!
    TRANSFER_ABORT_MAX_PLAYERS                      = 0x07,         // Transfer Aborted: instance is full
    TRANSFER_ABORT_NEED_GROUP                       = 0x0E,         // 3.1
    TRANSFER_ABORT_REALM_ONLY                       = 0x10,         // All players on party must be from the same realm.
    TRANSFER_ABORT_MAP_NOT_ALLOWED                  = 0x13,         // Map can't be entered at this time.
    TRANSFER_ABORT_TOO_MANY_INSTANCES               = 0x15,         // You have entered too many instances recently.
    TRANSFER_ABORT_TOO_MANY_REALM_INSTANCES         = 0x16,         // Additional instances cannot be launched, please try again later.
    TRANSFER_ABORT_XREALM_ZONE_DOWN                 = 0x17,
    TRANSFER_ABORT_ERROR                            = 0x19,
    TRANSFER_ABORT_INSUF_EXPAN_LVL                  = 0x1A,         // You must have <TBC, WotLK> expansion installed to access this area.
    TRANSFER_ABORT_ALREADY_COMPLETED_ENCOUNTER      = 0x1C,         // 4.2.2
    TRANSFER_ABORT_DIFFICULTY                       = 0x1E,         // <Normal, Heroic, Epic> difficulty mode is not available for %s.
};
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        Creature* GetNPCIfCanInteractWith(uint64 guid, uint32 npcflagmask, uint32 npcflagmask2 = 0);
        ...
        bool CanGetItemForLoot(ItemTemplate const* item) const;
        ...
        */
        ...
        void ModifyCurrency(uint32 id, int32 count, bool printLog = true, bool ignoreMultipliers = false, bool modifyWeek = true, bool modifySeason = true, bool refund = false);
        ...
        void KilledMonster(CreatureTemplate const* cInfo, uint64 guid, bool runAchiev = true);
        void KilledMonsterCredit(uint32 entry, uint64 guid = 0, bool runAchiev = true);
        ...
        void ClearComboPoints(bool clearComboTarget = false);
        ...
        TeamId GetBGTeamId() const { if (GetBGTeam() == ALLIANCE) return TEAM_ALLIANCE; if (GetBGTeam() == HORDE) return TEAM_HORDE; return TEAM_NEUTRAL; }
...
template <class T> T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, Spell* spell)
{
    ...
    for (auto itr : tempModList)
    {
        ...
        else if (mod->type == SPELLMOD_PCT)
        {
            ...
            if (mod->op == SPELLMOD_CASTING_TIME && mod->value < 0)
            {
                ...
                //TC_LOG_DEBUG("spell", "Player::ApplySpellMod SPELLMOD_CASTING_TIME totalflat %i totalmul %f basevalue %i spellId %i mod->spellId %i", totalflat, (totalmul - 1.0f), basevalue, spellId, mod->spellId);
                continue;
            }
            if (mod->op == SPELLMOD_COST && mod->value < 0)
            {
                ...
                //TC_LOG_DEBUG("spell", "Player::ApplySpellMod SPELLMOD_COST totalflat %i totalmul %f basevalue %i spellId %i mod->spellId %i", totalflat, (totalmul - 1.0f), basevalue, spellId, mod->spellId);
                continue;
            }
    ...
    if (modCast)
    {
        ...
        //TC_LOG_DEBUG("spell", "Player::ApplySpellMod totalflat %i totalmul %f basevalue %i, modCast->spellId %i", totalflat, (totalmul - 1.0f), basevalue, modCast->spellId);
    }
    if (modCost)
    {
        totalmul += CalculatePct(1.0f, modCost->value);
        DropModCharge(modCost, spell);
        //TC_LOG_DEBUG("spell", "Player::ApplySpellMod totalflat %i totalmul %f basevalue %i, modCost->spellId %i", totalflat, (totalmul - 1.0f), basevalue, modCost->spellId);
    }
