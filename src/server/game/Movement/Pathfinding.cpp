...
void Pathfinding::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        if (waterPath)
        {
            ...
            {
                ZLiquidStatus status = _pathParameter->pBaseMap->getLiquidStatus(GetPhaseShift(), _pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z, MAP_ALL_LIQUIDS, NULL);
    ...
    if (farFromPoly)
    {
        ...
        if (_pathParameter->typeID == TYPEID_UNIT)
        {
            ...
            if (_pathParameter->pBaseMap->IsUnderWater(GetPhaseShift(), p.x, p.y, p.z))
            {
...
void Pathfinding::UpdateAllowedPositionZ(float x, float y, float& z) const
{
    ...
    case TYPEID_UNIT:
    {
        ...
        if (!_pathParameter->canFly)
        {
            ...
            float ground_z = z;
            float max_z = canSwim
                ? _pathParameter->pMap->GetWaterOrGroundLevel(_pathParameter->GetPhaseShift(), x, y, z, &ground_z, !_pathParameter->hasAuraType)
                : ((ground_z = _pathParameter->pMap->GetHeight(x, y, z, true)));
        ...
        {
            float ground_z = _pathParameter->pMap->GetHeight(x, y, z, true);
            if (z < ground_z)
                z = ground_z;
        }
        break;
    ...
    case TYPEID_PLAYER:
    {
        ...
        {
            float ground_z = z;
            float max_z = _pathParameter->pMap->GetWaterOrGroundLevel(_pathParameter->GetPhaseShift(), x, y, z, &ground_z, !_pathParameter->hasAuraType);
            if (max_z > INVALID_HEIGHT)
            {
                if (z > max_z)
                    z = max_z;
...
{
    LiquidData data;
    ZLiquidStatus liquidStatus = _pathParameter->pBaseMap->getLiquidStatus(GetPhaseShift(), x, y, z, MAP_ALL_LIQUIDS, &data);
