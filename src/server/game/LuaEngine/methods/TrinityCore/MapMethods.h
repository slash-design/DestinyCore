...
namespace LuaMap
{
    ...
    {
        // STUBBED: Map::GetHeight signature changed in Legion (PhaseShift)
        E->CHECKVAL<float>(2);
        E->CHECKVAL<float>(3);
        E->CHECKVAL<uint32>(4, 1);
        E->Push();
        return 1;
    }
    ...
    {
        // STUBBED: Map::GetAreaId signature changed in Legion (PhaseShift)
        E->CHECKVAL<float>(2);
        E->CHECKVAL<float>(3);
        E->CHECKVAL<float>(4);
        E->CHECKVAL<uint32>(5, 0);
        E->Push();
        return 1;
    }
    ...
    {
        // STUBBED: HIGHGUID_* enums replaced by HighGuid:: scoped enums in Legion
        E->CHECKVAL<ObjectGuid>(2);
        E->Push();
        return 1;
    }
