...
namespace LuaUnit
{
    ...
    */
    ...
    {
        E->Push(unit->HasUnitState(UNIT_STATE_ROOT) || unit->HasUnitMovementFlag(MOVEMENTFLAG_ROOT));
        return 1;
    }
    ...
    */
    ...
    int GetFriendlyUnitsInRange(Eluna* E, Unit* unit)
    {
        ...
        ElunaUtil::ObjectGUIDCheck guidCheck(unit->GetGUID());
    ...
    {
        // STUBBED: CastSpellExtraArgs not available in Legion
        E->CHECKOBJ<Unit>(2, false);
        E->CHECKVAL<uint32>(3);
        E->CHECKVAL<bool>(4, false);
        E->CHECKVAL<int32>(5, 0);
        E->CHECKVAL<int32>(6, 0);
        E->CHECKVAL<int32>(7, 0);
        E->CHECKOBJ<Item>(8, false);
        E->CHECKVAL<ObjectGuid>(9, ObjectGuid());
        return 0;
    }
    ...
    {
        // STUBBED: Unit::DealDamage 8-arg signature, IsDamageReducedByArmor, SpellNonMeleeDamage ctor differ in Legion
        E->CHECKOBJ<Unit>(2);
        E->CHECKVAL<uint32>(3);
        E->CHECKVAL<bool>(4, true);
        E->CHECKVAL<uint32>(5, MAX_SPELL_SCHOOL);
        E->CHECKVAL<uint32>(6, 0);
        return 0;
    }
    ...
    {
        // STUBBED: Unit::Kill static helper signature differs in Legion
        E->CHECKOBJ<Unit>(2);
        E->CHECKVAL<bool>(3, true);
        return 0;
    }
    ...
    {
        // STUBBED: Unit::HandleStatFlatModifier / UnitModifierFlatType not available in Legion
        E->CHECKVAL<uint32>(2);
        E->CHECKVAL<uint8>(3);
        E->CHECKVAL<float>(4);
        E->CHECKVAL<bool>(5, true);
        return 0;
    }
    ...
    {
        // STUBBED: Unit::ApplyStatPctModifier / UnitModifierPctType not available in Legion
        E->CHECKVAL<uint32>(2);
        E->CHECKVAL<uint8>(3);
        E->CHECKVAL<float>(4);
        return 0;
