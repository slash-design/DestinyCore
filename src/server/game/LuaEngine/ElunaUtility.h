...
// Legion's TC_LOG -> outMessage -> Trinity::StringFormat uses fmt::sprintf (printf-style),
// so we pass the already-formatted message through "%s" instead of "{}".
#define ELUNA_LOG_TC_FMT(TC_LOG_MACRO, ...) \
    try { \
        std::string message = fmt::sprintf(__VA_ARGS__); \
        TC_LOG_MACRO("eluna", "%s", message.c_str()); \
    } catch (const std::exception& e) { \
        TC_LOG_MACRO("eluna", "Failed to format log message: %s", e.what()); \
    }
