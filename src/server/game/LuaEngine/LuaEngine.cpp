...
void Eluna::Push(LocalizedString const& str)
{
    char const* s = str.Str[DEFAULT_LOCALE];
    Push(s ? s : "");
}

void Eluna::Push(std::vector<SpellPowerCost> const& /*costs*/)
{
    Push(); // nil - SpellPowerCost not supported on Legion
}

void Eluna::Push(std::array<uint32, 2> const& arr)
{
    lua_newtable(L);
