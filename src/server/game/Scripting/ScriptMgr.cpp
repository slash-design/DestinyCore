...
void ScriptMgr::OnPlayerMoneyChanged(Player* player, int64& amount)
{
    ...
    if (Eluna* e = player->GetEluna())
    {
        ...
        e->OnMoneyChanged(player, amount32);
        amount = amount32;
    }
#endif
...
#ifdef ELUNA
    if (Eluna* e = player->GetEluna())
        e->OnUpdateZone(player, newArea ? newArea->GetId() : 0, oldArea ? oldArea->GetId() : 0);
#endif
...
void ScriptMgr::OnGuildMemberWitdrawMoney(Guild* guild, Player* player, uint64 &amount, bool isRepair)
{
    ...
    if (Eluna* e = sWorld->GetEluna())
    {
        ...
        e->OnMemberWitdrawMoney(guild, player, amount32, isRepair);
        amount = amount32;
    }
#endif
...
void ScriptMgr::OnGuildMemberDepositMoney(Guild* guild, Player* player, uint64 &amount)
{
    ...
    {
        uint32 amount32 = static_cast<uint32>(amount);
        e->OnMemberDepositMoney(guild, player, amount32);
        amount = amount32;
    }
#endif
