...
bool BotShamanAI::ProcessDispel()
{
	...
	if (friends.empty())
		return false;
    Trinity::Containers::RandomShuffle(friends);;
...
bool BotShamanAI::ProcessTotem()
{
	...
	{
		if (m_NonFearTotemTick > 0 || !ExistTotemByType(SUMMON_TYPE_TOTEM_EARTH, BOTAI_TOTEMRANGE))
		{
	...
	if (ShamanTotem_AbsorbBuff && !ExistTotemByType(SUMMON_TYPE_TOTEM_AIR, BOTAI_TOTEMRANGE))
	{
